...
parameter_names = ["C_m"] + list(meta["parameter_names"]) + ["vsini"]
min_labels = jnp.array(jnp.hstack([-1.5, meta["min_parameters"]]))
max_labels = jnp.array(jnp.hstack([+1.0, meta["max_parameters"]]))
...
def predict_x_m_difference(θ, f_modes, mean, coef, intercept):
    A = nufft.fourier_matvec(θ.reshape((-1, 1)), f_modes)
    # TODO: hack. we need the real x_m not the domain one
    x_m = ((θ[0] % jnp.pi) / jnp.pi) * (1 - (-1.5)) + (-1.5)
    return jnp.abs(x_m) * ((A - mean) @ coef + intercept)

NO_X_M_ARGS = object()

def emulate_spectrum(θ, f_modes, X, H, c_m_args=NO_X_M_ARGS):
    ...
    if c_m_args is not NO_X_M_ARGS:
        absorption += predict_x_m_difference(θ, *c_m_args)
...
@jax.jit
def fit_spectrum(flux, ivar, initial_parameters, vr_array, A, f_modes, X, H, c_m_args):
    θ_stellar_0 = to_domain(initial_parameters)

    emulate_args = (f_modes, X, H, c_m_args)
...
# todo; return modes or f_modes?
c_m_args = (c_m_args[0], c_m_args[1], c_m_args[2][:, :8574], c_m_args[3][:8574])

emulate_args = (jnp.array(f_modes), jnp.array(X), jnp.array(H), c_m_args)
args = (vr_array, jnp.array(A), *emulate_args)


param_dict = dict(Teff=5750, logg=4.5, m_H=0.0, vmic=0.8, vsini=20.0, C_m=0.0)
...
solar_initial_parameters_dict = dict(Teff=5750, logg=4.3, m_H=0.0, vmic=0.8, vsini=20.0, C_m=0.0)
...
comparison_parameters_dict=dict(Teff=5738.0, logg=4.41, m_H=0.01, vmic=-0.37, vsini=14.2, C_m=+0.50)
