...
output_path_prefix = "korg_grid_subset_4d_mp"
...
with h5.File(input_path, "r") as fp:
    parameter_names = ("vmic", "m_H", "logg", "Teff")
    slices = (
        slice(0, 1), # N_m
        slice(0, 1), # C_m
        slice(0, 1), # alpha_m
    ...
    min_parameters = np.array([np.min(fp[k][s]) for k, s in zip(parameter_names, slices[3:])])
    max_parameters = np.array([np.max(fp[k][s]) for k, s in zip(parameter_names, slices[3:])])
    grid_parameters = [fp[k][s] for k, s in zip(parameter_names, slices[3:])]    
    ...
    *   (fp["converged_flag"][slices] == 1)
    ...
    flux = flux.reshape(flux.shape[3:])
    ...
    with open("marcs_filled_atmosphere_paths.txt", "r") as marcs_fp:
        for line in marcs_fp:
            ...
            if (
                (r["C_m"] != fp["C_m"][0])
            or  (r["alpha_m"] != fp["alpha_m"][0])
            or not (max_parameters[teff_index] >= r["Teff"] >= min_parameters[teff_index])
            ):
            ...
            |   (not is_plane_parallel and r["vmic"] == 2.0)
            ):
